...
This thesis examines topical consistency between content and fact-checks in the Birdwatch dataset published by Twitter. We find topics by clustering word type embeddings (following a method introduced by \cite{sia-etal-2020-tired}) to evaluate two research questions (1) Do notes that have high topic overlap with their associated tweet get better ratings? and (2) Can this method capture the helpful extra context that Birdwatch notes add to a
tweet? Kullback-Leibler divergence is used to proxy topic overlap between the documents. We find that there is a statistically significant relationship between topic overlap and helpfulness but cannot establish a relationship between helpfulness and added context.
...
Engaging with fact-checks is an increasingly standard part of our experience on social media. At least two social media companies launched community based, crowd-sourced efforts to fact-check their content in recent years: Facebook's community review in 2019 and Twitter's Birdwatch program in 2021 \citep{yasseri2021can}. These efforts run in parallel to automatic fact-checking methods, but have not received much attention from the natural language processing community yet. Twitter has open-sourced the Birdwatch dataset, a resource that has claims (tweets), fact-checks (Birdwatch notes), and labels (ratings). We use the Birdwatch dataset to investigate a reasonably intuitive assumption about a crowd-sourced misinformation platform: fact-checks, that are rated by the community as helpful, should share topics with the content that they're fact-checking.
...
% \cite{sia-etal-2020-tired} found that using the KMeans algorithm with term frequency weights to create the clusters, and using the weights to re-rank the top word types is effective.
...
% NPMI is a metric based on vector semantics and provides a measure of similarity based on how often words co-occur.
...
% Being on-topic is a less strong lever for helpfulness when considering notes marked with the “Provides Important Context” label.
...
% The relationship between $KL(N || T)$ and helpfulness for the notes that add context is weaker than in the full dataset. This may reflect that the labeled context-adding notes have less topic overlap but are still helpful.
